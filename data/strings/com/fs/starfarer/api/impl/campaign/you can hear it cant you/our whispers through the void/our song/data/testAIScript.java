...
import data.test.testHullMod.SharedAIData;
...
public class testAIScript implements MissileAIPlugin {

    private static float SOURCE_REJOIN = 200f;  //distance added to the collision radius that determines target radius for motes returning
                                                //towards their host ship
    private static float SOURCE_REPEL = 50f;    //distance added to the collision radius that determines the radius below which motes
    ...
    public testAIScript(MissileAPI missile) {
        this.missile = missile;
        r = (float) Math.random();
        elapsed = -(float) Math.random() * 0.5f;

        data = testHullMod.getSharedData(missile.getSource());
...
//			}
    ...
    protected void acquireNewTargetIfNeeded() {
        ...
        float maxDistFromSourceShip = testHullMod.MAX_DIST_FROM_SOURCE_TO_ENGAGE_AS_PD;
        float maxDistFromAttractor = testHullMod.MAX_DIST_FROM_ATTRACTOR_TO_ENGAGE_AS_PD;
    ...
    protected int getNumMotesTargeting(CombatEntityAPI other) {
        ...
        for (MissileAPI mote : data.drones) {
            ...
            if (mote.getUnwrappedMissileAI() instanceof testAIScript) {
                testAIScript ai = (testAIScript) mote.getUnwrappedMissileAI();
