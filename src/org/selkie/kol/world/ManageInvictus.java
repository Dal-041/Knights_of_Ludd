...
public class ManageInvictus implements FleetEventListener {
	...
	public void reportBattleOccurred(CampaignFleetAPI fleet, CampaignFleetAPI primaryWinner, BattleAPI battle) {
		...
		if(Global.getSector().getMemoryWithoutUpdate().contains(MEMKEY_SSSSS_LP_INVICTUS_DROPPED) 
	                && Global.getSector().getMemoryWithoutUpdate().getBoolean(MEMKEY_SSSSS_LP_INVICTUS_DROPPED)){
	            return;
		}
	        
		if(fleet.getFlagship()==null || !fleet.getFlagship().getHullSpec().getBaseHullId().startsWith("ssssss_invictus_lp")){
			...
			for (FleetMemberAPI f : Global.getSector().getPlayerFleet().getFleetData().getMembersListCopy()){
				if(f.getHullId().startsWith("ssssss_invictus_lp")) salvaged=true;
	                
	                //set memkey that the wreck must never spawn
	                Global.getSector().getMemoryWithoutUpdate().set(MEMKEY_SSSSS_LP_INVICTUS_DROPPED,true); 
	        }
